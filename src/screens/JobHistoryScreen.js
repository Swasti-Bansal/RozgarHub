...
// Displays the history of past jobs completed / cancelled by the logged-in worker
      ...
      <ScrollView style={styles.screenContainer} showsVerticalScrollIndicator={false}>
        ...
        <View style={localStyles.summaryRow}>
          <View style={localStyles.summaryCard}>
            ...
            <Text style={localStyles.summaryLabel}>Total Jobs</Text>
          </View>
          <View style={[localStyles.summaryCard, localStyles.summaryCardHighlight]}>
            ...
            <Text style={localStyles.summaryLabel}>Total Earned</Text>
          </View>
          <View style={localStyles.summaryCard}>
            <Text style={localStyles.summaryValue}>⭐ {SUMMARY.avgRating}</Text>
            <Text style={localStyles.summaryLabel}>Avg Rating</Text>
          </View>
        </View>
        ...
        <View style={localStyles.filterRow}>
          {FILTER_TABS.map((tab) => (
            ...
            >
              <Text style={[localStyles.filterTabText, activeFilter === tab && localStyles.filterTabTextActive]}>
                {tab}
              </Text>
            </TouchableOpacity>
          ))}
        ...
        {filtered.length === 0 ? (
          <View style={localStyles.emptyState}>
            ...
            <Text style={localStyles.emptyTitle}>No jobs found</Text>
            <Text style={localStyles.emptySubtitle}>No {activeFilter.toLowerCase()} jobs in your history.</Text>
          </View>
        ) : (
          filtered.map((job) => {
              ...
              >
                ...
                <View style={localStyles.cardHeader}>
                  <View style={localStyles.cardTitleBlock}>
                    <Text style={localStyles.cardTitle}>{job.title}</Text>
                    <Text style={localStyles.cardEmployer}>🏢 {job.employer}</Text>
                  </View>
                  <View style={[localStyles.statusTag, { backgroundColor: sc.bg, borderColor: sc.border }]}>
                    <Text style={[localStyles.statusTagText, { color: sc.text }]}>{job.status}</Text>
                  </View>
                </View>

                {/* Meta */}
                <View style={localStyles.cardMeta}>
                  <Text style={localStyles.cardMetaItem}>📍 {job.location}</Text>
                ...
                <View style={localStyles.cardFooter}>
                  <View>
                    ...
                    <Text style={localStyles.cardDuration}>⏱️ {job.duration}</Text>
                  </View>
                  {job.workerRating !== null ? (
                    ...
                    job.status === 'Completed' && (
                      <TouchableOpacity
                        style={localStyles.rateNowBtn}
                        onPress={() => navigation.navigate('RateJob')}
                      >
                        <Text style={localStyles.rateNowText}>Rate Now</Text>
                      </TouchableOpacity>
                    )
                  )}
                </View>
                ...
                <View style={localStyles.viewDetailRow}>
                  <Text style={localStyles.viewDetailText}>View Details →</Text>
                </View>
              </TouchableOpacity>
            );
          })
